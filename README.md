Secure Chat — INFOSEC Assignment 2

Fall 2025 — Secure Communication System with PKI, DH, AES-128, RSA Signatures, MySQL & Non-Repudiation

This repository contains the complete implementation of the Secure Chat assignment for the INFOSEC course.
It includes a custom Public Key Infrastructure (PKI), authenticated client–server communication, DH-based ephemeral key exchange, encrypted & signed chat messages, tamper/replay protection, user registration/login with database, transcripts, signed session receipts, and attack/defense testing (tamper, replay, invalid-cert).
...
├── .env.example
│
├── certs/                 # (ignored) Contains CA, server, and client certs/keys
│
├── scripts/
│   ├── gen_ca.py          # Generate CA keypair + self-signed cert
│   ├── gen_cert.py        # Generate leaf certs (server/client) signed by CA
│   ├── tamper_resend.py   # Tamper/replay attack script
│   ├── verify_session.py  # Verify transcript integrity + receipt signatures
│   ├── verify_all.sh      # End-to-end receipt/transcript verification
│   └── package_submission.sh
│
├── lib/
│   ├── crypto_utils.py    # DH, AES-128-CBC, RSA sign/verify helpers
│   ├── cert_utils.py      # Certificate parsing & fingerprint utilities
│   └── transcript.py      # Append-only transcripts + signed receipts
│
├── db/
│   ├── db.py              # MySQL interface (registration/login)
│   └── secure_chat_schema.sql
│
├── auth/
│   ├── auth_server.py     # Handles registration + login over DH-protected channel
│   └── auth_client.py
│
├── chat/
│   ├── chat_server.py     # Main secure chat server
│   └── chat_client.py     # Chat client (encrypted + signed messages)
│
├── transcripts/           # Autogenerated logs + signed receipts (per session)
└── docs/
    └── TestReport.docx    # Final report with screenshots + PCAP evidence
...
Step 3 — Start MySQL and initialize DB schema
mysql -u root -p < db/secure_chat_schema.sql

Step 4 — Run authentication server + client (registration/login)
...
Client → Server: ephemeral DH public value + certificate + signature.

Server → Client: DH reply + certificate + signature.

Both derive same shared secret → AES-128 key.
...
📄 5. Test Report (docs/TestReport.docx)
...
✔️ 7. Assignment Requirements Checklist
Requirement	Status
PKI (CA + certs)	✔️
Hello exchange with cert verification	✔️
DH session key establishment	✔️
AES-128 encrypted messaging	✔️
RSA signatures on messages	✔️
Replay protection	✔️
Tamper detection	✔️
Registration & login DB	✔️
Transcripts	✔️
Signed receipts (non-repudiation)	✔️
PCAP evidence	✔️
TestReport.docx	✔️
Packaging script	✔️
❤️ Credits
